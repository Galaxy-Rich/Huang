        ...
        if l then
            ...
            task.spawn(function()
                while getgenv().PushStrengthEnabled do
                    ...
                    task.wait()  -- Menambahkan wait untuk menghindari lag berlebih, bisa disesuaikan
                end
            end)
        end
    end
...
---
-- Pet Trading Section
local petSection = Crystal:Section({Title = "Pet Infinity", Icon = "infinity"})
...
Crystal:Toggle({
    ...
    Callback = function(state)
        ...
        if state then task.spawn(autoBuyPetLoop) end
    end
})
...
Crystal:Toggle({
    ...
    Callback = function(state)
        ...
        if state then task.spawn(autoEvolvePetLoop) end
    end
})
...
local function equipPunchTool()
	...
	local punch = backpack and backpack:FindFirstChild("Punch") or char and char:FindFirstChild("Punch")
...
local function updateStatus(message)
    print("[KILL AURA]: " .. message)
end

-- FUNGSI AUTO PUNCH DIHAPUS AGAR BISA MANUAL --
...
local function createCircleVisual(radius)
    for _, part in ipairs(radiusVisuals) do
        part:Destroy()
    end
    radiusVisuals = {}
    ...
    for i = 1, segments do
        ...
        segment.Size = Vector3.new(0.5, 0.1, 0.5)
        segment.TopSurface = Enum.SurfaceType.Smooth
        segment.BottomSurface = Enum.SurfaceType.Smooth
        table.insert(radiusVisuals, segment)
    end
    return radiusVisuals
end

-- ====== FUNGSI UPDATE POSISI LINGKARAN ======
local function updateCirclePosition(root)
    if not root or #radiusVisuals == 0 then return end
    
    local radius = currentRadius
    local segments = #radiusVisuals
    local angleStep = (2 * math.pi) / segments
    
    for i, segment in ipairs(radiusVisuals) do
        local angle = i * angleStep
        local x = math.cos(angle) * radius
        local z = math.sin(angle) * radius
        
        segment.CFrame = root.CFrame * CFrame.new(x, -6.5, z)
        if segment.Parent ~= workspace then
...
KillSelect:Input({
    ...
    Callback = function(text)
        ...
        if n then
            ...
            if runAura and #radiusVisuals > 0 then
                local char = LocalPlayer.Character
                local root = char and char:FindFirstChild("HumanoidRootPart")
                createCircleVisual(currentRadius)
                if root then updateCirclePosition(root) end
            end
            updateStatus("Aura Radius set to " .. currentRadius)
        end
    end
})
        ...
        if state then
            ...
            auraConnection = RunService.Heartbeat:Connect(function()
                ...
                if root and rHand and lHand then
                    updateCirclePosition(root)

                    -- HANYA MENGIRIM TOUCH INTEREST (HITBOX)
                    -- Kamu harus menekan tombol PUNCH secara manual agar damage masuk
                    for _, p in ipairs(Players:GetPlayers()) do
                        if p ~= LocalPlayer and not friendWhitelist[p.UserId] and p.Character then
                            ...
                            if enemyRoot and enemyHum and enemyHum.Health > 0 then
                                ...
                                if dist <= currentRadius then
                                    firetouchinterest(rHand, enemyRoot, 1)
                                    firetouchinterest(rHand, enemyRoot, 0)
                                    firetouchinterest(lHand, enemyRoot, 1)
                                    firetouchinterest(lHand, enemyRoot, 0)
                                end
                            end
                        end
                    end
                else
                    for _, segment in ipairs(radiusVisuals) do
                        segment.Parent = nil
                    end
                end
            end)
            updateStatus("Kill Aura ACTIVATED (Manual Punch)")
        else
            for _, segment in ipairs(radiusVisuals) do
                segment:Destroy()
            end
            radiusVisuals = {}
            updateStatus("Kill Aura DEACTIVATED")
        end
    end
})
