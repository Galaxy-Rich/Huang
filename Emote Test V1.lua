--[[ 
    ...
    Modified with Master Toggle Logic for all extra buttons.
    REMOVED: Emote Walk
]]
...
local Under, UIListLayout, _1left, _9right, _4pages, _3TextLabel, _2Routenumber, Top, UICorner1,
...
if Reload then
    ...
    Reload.Visible = speedEmoteEnabled and (currentMode == "animation")
end
end
...
local function updateAllExtraButtonsVisibility()
    if SpeedBox then SpeedBox.Visible = speedEmoteEnabled end
    if Changepage then Changepage.Visible = speedEmoteEnabled end
    if Favorite then Favorite.Visible = speedEmoteEnabled end
    if Reload then 
        Reload.Visible = speedEmoteEnabled and (currentMode == "animation") 
    end
end
...
local function loadSpeedEmoteConfig()
    if readfile and isfile and isfile(speedEmoteConfigFile) then
        ...
        if success and result then
            ...
            updateAllExtraButtonsVisibility()
        end
    end
end
...
local function createGUIElements()
    ...
    if emotesWheel:FindFirstChild("1left") then
        emotesWheel["1left"]:Destroy()
    end
    if emotesWheel:FindFirstChild("9right") then
        emotesWheel["9right"]:Destroy()
    end
    ...
    _1left.Position = UDim2.new(-0.10, 0, 0.5, 0) 
    _1left.AnchorPoint = Vector2.new(0.5, 0.5) 
    _1left.Rotation = 0 
    _1left.Size = UDim2.new(0.2, 0, 0.2, 0) 
    _1left.Image = "rbxassetid://93111945058621"
    _1left.ImageColor3 = Color3.fromRGB(0, 0, 0)
    _1left.ImageTransparency = 0.400

    _9right.Name = "9right"
    _9right.Parent = emotesWheel 
    ...
    _9right.Position = UDim2.new(1.10, 0, 0.5, 0) 
    _9right.AnchorPoint = Vector2.new(0.5, 0.5) 
    _9right.Rotation = 0 
    _9right.Size = UDim2.new(0.2, 0, 0.2, 0) 
    ...
    _4pages.Position = UDim2.new(1.07, 0, -5.4, 0)
    _4pages.Size = UDim2.new(0.5, 0, 0.811320841, 0)
    _4pages.Font = Enum.Font.SourceSansBold
    _4pages.Text = "1"
    _4pages.TextColor3 = Color3.fromRGB(0, 0, 0)
    _4pages.TextScaled = true
    _4pages.TextSize = 30.000
    ...
    _2Routenumber.Position = UDim2.new(-0.4, 0, -5.4, 0)
    ...
    Search.PlaceholderText = "Nama Emote/ID"
    Search.Text = ""
    ...
    Favorite.Position = UDim2.new(1.6, 0, -0.17, 0)
    ...
    SpeedBox.Position = UDim2.new(1.5, 0, -0.17, 0)
    ...
    SpeedEmote.Position = UDim2.new(1.4, 0, -0.17, 0)
...
Changepage.Position = UDim2.new(1.3, 0, -0.17, 0)
...
Reload.Position = UDim2.new(1.2, 0, -0.17, 0)
...
local function checkAndRecreateGUI()
    ...
    if not emotesWheel:FindFirstChild("Under") or not emotesWheel:FindFirstChild("Top") or
        not emotesWheel:FindFirstChild("Favorite") or not emotesWheel:FindFirstChild("SpeedEmote") or 
        not emotesWheel:FindFirstChild("SpeedBox") or not emotesWheel:FindFirstChild("Changepage") or 
        not emotesWheel:FindFirstChild("Reload") or not emotesWheel:FindFirstChild("1left") or 
        not emotesWheel:FindFirstChild("9right") then
        isGUICreated = false
    ...
    while true do
        ...
        if not emotesMenu then
            ...
            if exists then
                ...
                if not emotesWheel:FindFirstChild("Under") or not emotesWheel:FindFirstChild("Top") or 
                   not emotesWheel:FindFirstChild("1left") or not emotesWheel:FindFirstChild("9right") then
                    if createGUIElements then
                        createGUIElements()
                        loadSpeedEmoteConfig()
                    end
